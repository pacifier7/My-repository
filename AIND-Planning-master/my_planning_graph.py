...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            node_act = PgNode_a(action)
            if node_act.prenodes.issubset(self.s_levels[level]):
                actnodes.append(node_act)
                for node_s in self.s_levels[level]:
                    node_s.children.add(node_act)
                    node_act.parents.add(node_s)
    ...
    def add_literal_level(self, level):
        ...
        statenodes = set()
        for node_act in self.a_levels[level - 1]:
            for node_s in node_act.effnodes:
                statenodes.add(node_s)
                node_s.parents.add(node_act)
                node_act.children.add(node_s)
        self.s_levels.append(statenodes)
